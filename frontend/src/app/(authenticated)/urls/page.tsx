    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">Manage your short links.</p>
        </div>
        <Link href="/urls/new">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search by original URL or short code..." value={search} onChange={(e) => setSearch(e.target.value)} className="border-zinc-800 bg-zinc-900/50 pl-9 text-sm placeholder:text-zinc-600 focus:border-blue-500" />
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {(search || folderFilter || tagFilter) && (
            <Button variant="ghost" size="sm" onClick={() => { setSearch(""); setFolderFilter(""); setTagFilter("") }} className="text-zinc-400">
      ...
      <Card className="border-zinc-800/50">
        <CardHeader className="border-b border-zinc-800/50 pb-3">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            ...
            All URLs
            {items.length > 0 && (
              <span className="ml-auto text-xs font-normal text-zinc-500">{items.length} link{items.length !== 1 ? "s" : ""}</span>
            )}
          </CardTitle>
        </CardHeader>
              ...
              {items.map((url) => (
                <div key={url.id} className="group flex flex-col gap-2 px-6 py-4 transition-colors hover:bg-zinc-900/30 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    {url.tags && url.tags.length > 0 && (
                      <div className="mt-1.5 flex flex-wrap gap-1">
                        {url.tags.map((t) => (
                          <span key={t} className="inline-flex items-center gap-1 rounded-md bg-zinc-800/50 px-1.5 py-0.5 text-[10px] text-zinc-400">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => favoriteSet.has(url.id) ? removeFavorite.mutate(url.id) : addFavorite.mutate(url.id)} className={`rounded-lg p-2 transition-colors ${favoriteSet.has(url.id) ? "text-red-400 hover:bg-red-500/10" : "text-zinc-500 hover:text-red-400 hover:bg-red-500/10"}`}>
                      <Heart className={`size-3.5 ${favoriteSet.has(url.id) ? "fill-current" : ""}`} />
                    </button>
                    <Link href={`/urls/${url.id}`} className="rounded-lg p-2 text-zinc-500 hover:text-blue-400 hover:bg-blue-500/10 transition-colors">
                      <ExternalLink className="size-3.5" />
                    </Link>
                    <Link href={`/urls/${url.id}/analytics`} className="rounded-lg p-2 text-zinc-500 hover:text-green-400 hover:bg-green-500/10 transition-colors">
                      <BarChart3 className="size-3.5" />
                    </Link>
                    <button onClick={() => deleteUrl.mutate(url.id)} className="rounded-lg p-2 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">
